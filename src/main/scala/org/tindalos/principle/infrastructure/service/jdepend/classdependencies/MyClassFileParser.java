...
public class MyClassFileParser extends AbstractParser {
     ...
     * <code>MyJavaClass</code> was parsed.
     */
    public MyJavaClass parse(File classFile) throws IOException {
    ...
    @Override
    public MyJavaClass parse(InputStream is) throws IOException {

        reset();

        jClass = new MyJavaClass("Unknown");
    ...
    private Constant parseNextConstant() throws IOException {
        ...
        switch (tag) {
            ...
            case (MyClassFileParser.CONSTANT_CLASS):
            case (MyClassFileParser.CONSTANT_STRING):
            case (MyClassFileParser.CONSTANT_METHOD_TYPE):
                result = new Constant(tag, in.readUnsignedShort());
                break;
            case (MyClassFileParser.CONSTANT_FIELD):
            case (MyClassFileParser.CONSTANT_METHOD):
            case (MyClassFileParser.CONSTANT_INTERFACEMETHOD):
            case (MyClassFileParser.CONSTANT_NAMEANDTYPE):
            case (MyClassFileParser.CONSTANT_INVOKEDYNAMIC):
                result = new Constant(tag, in.readUnsignedShort(), in.readUnsignedShort());
                break;
            case (MyClassFileParser.CONSTANT_INTEGER):
                result = new Constant(tag, new Integer(in.readInt()));
                break;
            case (MyClassFileParser.CONSTANT_FLOAT):
                result = new Constant(tag, in.readFloat());
                break;
            case (MyClassFileParser.CONSTANT_LONG):
                result = new Constant(tag, in.readLong());
                break;
            case (MyClassFileParser.CONSTANT_DOUBLE):
                result = new Constant(tag, in.readDouble());
                break;
            case (MyClassFileParser.CONSTANT_UTF8):
                result = new Constant(tag, in.readUTF());
                break;
            case (MyClassFileParser.CONSTANT_METHOD_HANDLE):
    ...
    private String getPackageName(String s) {
        if (!s.contains("$") &&
                (s.startsWith(rootName.replaceAll("\\.", "/"))
                        || s.startsWith(rootName))) {
            jClass.addDependency(s.replaceAll("/", "\\."));
        }
    ...
    public String toString() {
        ...
        try {
            ...
            s.append("\nDependencies:\n");
            for (Object jPackage : jClass.getImportedPackages()) {
                s.append("    " + jPackage + "\n");
            }
