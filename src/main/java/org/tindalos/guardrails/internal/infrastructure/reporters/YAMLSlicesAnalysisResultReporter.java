...
import org.tindalos.guardrails.internal.app.reporters.SlicesAnalysisResultReporter;
...
/**
 * Reports slices analysis results in structured YAML format.
 */
public class YAMLSlicesAnalysisResultReporter implements SlicesAnalysisResultReporter {

    @Override
    public Class<SlicesAnalysisResult> resultType() {
        return SlicesAnalysisResult.class;
    }

    @Override
    public String report(SlicesAnalysisResult result) {
        var header = """
                slices_result:
    ...
    private String groupYaml(SliceGroupResult group) {
    ...
    private String dependenciesYaml(String key, Map<Slice, Set<Slice>> deps) {
        if (deps.isEmpty()) {
            return key + ": []\n";
        }
