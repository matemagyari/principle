...
non-local guesses:
	 -scala/collection/JavaConverters.input.packages.
	 -scala/collection/JavaConverters.input.packages#
	 -scala/collection/JavaConverters.input.packages().
	 -input/packages.
	 -input/packages#
	 -input/packages().
	 -scala/Predef.input.packages.
	 -scala/Predef.input.packages#
	 -scala/Predef.input.packages().
offset: 640
...
class CycleDetector(packageStructureBuilder: PackageStructureBuilder) extends Analyzer {
    ...
    override def analyze(input: AnalysisInput) = {
      ...
      val basePackage = packageStructureBuilder.build(input.packa@@ges().asInstanceOf[java.util.List[Package]], input.analysisPlan().basePackage)
