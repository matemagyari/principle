...
non-local guesses:
	 -org/junit/analysisRunner/run.
	 -org/junit/analysisRunner/run#
	 -org/junit/analysisRunner/run().
	 -org/tindalos/principle/domain/analyzers/adp/analysisRunner/run.
	 -org/tindalos/principle/domain/analyzers/adp/analysisRunner/run#
	 -org/tindalos/principle/domain/analyzers/adp/analysisRunner/run().
	 -org/tindalos/principle/domain/constraints/analysisRunner/run.
	 -org/tindalos/principle/domain/constraints/analysisRunner/run#
	 -org/tindalos/principle/domain/constraints/analysisRunner/run().
	 -scala/collection/JavaConverters.analysisRunner.run.
	 -scala/collection/JavaConverters.analysisRunner.run#
	 -scala/collection/JavaConverters.analysisRunner.run().
	 -analysisRunner/run.
	 -analysisRunner/run#
	 -analysisRunner/run().
	 -scala/Predef.analysisRunner.run.
	 -scala/Predef.analysisRunner.run#
	 -scala/Predef.analysisRunner.run().
offset: 4315
...
class ADPTest {
  ...
  private def run(basePackage: String): util.Map[PackageReference, util.Set[Cycle]] = {
    ...
    val result = analysisRunner.ru@@n(new AnalysisInput(packageInputs.asJava, classes.asJava, plan))
    assertEquals(1, result.size())
