...
non-local guesses:
	 -scala/collection/JavaConverters.input.packageCouplingExpectations.
	 -scala/collection/JavaConverters.input.packageCouplingExpectations#
	 -scala/collection/JavaConverters.input.packageCouplingExpectations().
	 -input/packageCouplingExpectations.
	 -input/packageCouplingExpectations#
	 -input/packageCouplingExpectations().
	 -scala/Predef.input.packageCouplingExpectations.
	 -scala/Predef.input.packageCouplingExpectations#
	 -scala/Predef.input.packageCouplingExpectations().
offset: 1486
...
class CycleDetector(packageStructureBuilder: PackageStructureBuilder) extends Analyzer {
    ...
    override def analyze(input: AnalysisInput) = {
      ...
      new ADPResult(cycles, input.packageCo@@uplingExpectations().flatMap(_.adp()).get)
    }
