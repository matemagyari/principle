error id: file://<WORKSPACE>/src/test/scala/org/tindalos/principle/domain/checker/ADPTest.scala:Cycle
file://<WORKSPACE>/src/test/scala/org/tindalos/principle/domain/checker/ADPTest.scala
empty definition using pc, found symbol in pc: 
semanticdb not found

found definition using fallback; symbol Cycle
offset: 2407
...
class ADPTest {
  ...
  def transitive2() = {
    ...
    val expectedValue: util.Set[Cycl@@e] = result.values().iterator().next()
    assertEquals(util.Collections.unmodifiableSet(expectedCycle), expectedValue)
  }
